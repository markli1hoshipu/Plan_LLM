#!/usr/bin/env python3
"""
Step 1 (Pass@k): Generate k raw responses per problem from LLM.

This script uses vLLM to generate k responses per problem for pass@k evaluation.
...
def generate_responses(args):
    """
    Generate k raw responses per problem using vLLM for pass@k evaluation.
    """
    ...
    print(f"Generating {args.num_samples} samples per problem for pass@{args.num_samples}")
    ...
    # Generation parameters - n=num_samples for pass@k
    ...
    if is_mistral:
        ...
        print(f"Generating {args.num_samples} predictions each for {len(input_token_ids)} examples...")
        ...
        print(f"Generating {args.num_samples} predictions each for {len(input_texts)} examples...")
        outputs = engine.generate(input_texts, sampling_params)

    # Process results - save k responses per problem
    raw_results = []
    ...
    for i, output in enumerate(outputs):
        ...
        # Collect all k responses
        raw_responses = []
        num_tokens = []
        for sample_output in output.outputs:
            raw_responses.append(sample_output.text)
            num_tokens.append(len(sample_output.token_ids))

        raw_results.append({
            "num_tokens": num_tokens,  # List of k token counts
            "initial_state": initial_state,
            "goal_state": goal_state,
            "gt_actions": ground_truths[i],
            "input_prompt": prompts[i][1]['content'],
            "raw_responses": raw_responses,  # List of k responses
        })
    ...
    print(f"\n{'='*60}")
    print(f"Step 1 (Pass@{args.num_samples}): Raw Response Generation Complete")
    print(f"{'='*60}")
    print(f"Model: {args.model}")
    print(f"Data: {args.data_file}")
    print(f"Problems: {len(raw_results)}")
    print(f"Samples per problem: {args.num_samples}")
    print(f"Total responses: {len(raw_results) * args.num_samples}")
...
def get_args():
    parser = argparse.ArgumentParser(
        description="Step 1 (Pass@k): Generate k raw responses per problem using vLLM",
    ...
    parser.add_argument("--num_samples", type=int, default=4,
                        help="Number of samples per problem (k in pass@k)")
    ...
    parser.add_argument("--disable_thinking", action="store_true",
                        help="Disable thinking mode (for Qwen3/Qwen3.5 models)")
